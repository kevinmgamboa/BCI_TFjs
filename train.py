...
for tra, val in kfold.split(database.data['train']['epochs'], database.data['train']['labels']):
    ...
    hub.build_model_structure(database.info['data_shape'])
    # compile model
    hub.compile()
    # initializing model
    ...
    for n_ep in range(train_epochs):
        print('------- train score -------')
        # Train the model ---------------------------------------------------------
        train_score = hub.model.fit(database.data['train']['epochs'][tra],
        ...
        test_scores = hub.model.evaluate(database.data['test']['epochs'], database.data['test']['labels'])
        # saves train history
